...
@partial(np.vectorize, signature="(c),()->()") # python decorator to vectorize a function
...
# a utility function to return a default TrainState object
...
def create_train_state(rng, model_cls, trainloader, lr=1e-3, lr_layer=None, lr_schedule=False, weight_decay=0.0, total_steps=-1,):
...
# Define separate step functions for running training and evaluation steps, accordingly. These step functions are
# each wrapped in a call to `@jax.jit` which fuses operations, generally leading to high performance gains.
...
def example_train(dataset: str, layer: str, seed: int, model: DictConfig, train: DictConfig,):
    ...
    trainloader, testloader, n_classes, l_max, d_input = create_dataset_fn(bsz=train.bsz)
    ...
    for epoch in range(train.epochs):
        ...
        test_loss, test_acc = validate(state.params, model_cls, testloader, classification=None)
...
@hydra.main(version_base=None, config_path="/content", config_name="config")
